      ...
      <el-row align="middle" :gutter="20">
        <el-col :xs="24" :sm="12">
          <el-row :gutter="20">
            ...
            <el-col :xs="24" :sm="8">
              <el-date-picker
                v-model="form._checkedOffDate"
                type="daterange"
                style="width: 100%"
                range-separator="至"
                start-placeholder="核销时间"
                end-placeholder="核销时间"
                @change="query()"
              >
              </el-date-picker>
            </el-col>
        ...
        <el-table-column width="160" label="操作">
          <template #default="scope">
            <el-button type="primary" plain @click="edit(scope)"
              >编辑</el-button
            >
            <el-button type="warning" plain @click="del(scope)"
              >作废---------</el-button
            >
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 列表
    query(form) {
      const filtersForm = form || this.form
      if (filtersForm._checkedOffDate) {
        filtersForm.checkedOffDateBegin = moment(
          filtersForm._checkedOffDate[0]
        ).valueOf()
        filtersForm.checkedOffDateEnd = moment(
          filtersForm._checkedOffDate[1]
        ).valueOf()
      }
      couponList(form || this.form).then((res) => {
        res.data.records.forEach((item) => {
          ...
          item.checkedOffDate = moment(item.checkedOffDate).format(
            'YYYY-MM-DD hh:mm:ss'
          )
          this.tableData = {
